...
Тег ставьте с ревизией: `X.Y.Z-N` (например `0.19.19-20`) → пакеты `X.Y.Z-rN`. Голый `X.Y.Z` даёт версию фида `X.Y.Z-r1`, не совпадающую с пакетами. `hwelp-proxy` собирается с той же версией (`tools/hwelp-version.sh`): community-фид owfeed отдаёт исходники релиза только пакетам с версией тега, а GPL-пакет без исходников блокирует публикацию.
...
В README, раздел «Установка», стоят прямые ссылки на пакеты релиза (в имени файла есть версия). После каждого релиза обновите в них тег и имена файлов, например `0.19.19/luci-app-podkop-bot-0.19.19-r17.apk` и `0.19.19/luci-app-podkop-bot_0.19.19-r17_all.ipk`. Однострочник `bearguard-install.sh` всегда берёт последний релиз и правки не требует.

Пакеты `hwelp-proxy` из артефакта сборки owfeed публикует в релиз сам, вместе с подписями; отдельный шаг для них не нужен.
