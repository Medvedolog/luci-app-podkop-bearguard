...
function hwelpNode(st){var pid=parseInt(st.pid||0,10)||0,s='';if(verified(st)){s=_('работает');if(pid>0)s+=' · PID '+String(pid);return dot('green',s);}if(st.running){s=(st.probing||st.state==='verifying'||st.starting)?_('запущен, проверяется'):_('запущен');if(pid>0)s+=' · PID '+String(pid);return dot('yellow',s);}if(st.starting)return dot('yellow',_('запускается'));if(st.state==='failed'&&(String(st.reason||'').indexOf('gateway')>=0||String(st.reason||'').indexOf('proxy')>=0))return dot('red',_('ошибка запуска'));return dot('grey',_('не запущен'));}
