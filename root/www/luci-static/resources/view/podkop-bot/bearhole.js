...
return view.extend({
	...
	progressCard:function(st){if(!(st.starting||st.probing||st.state==='verifying'))return E('span',{});var done=parseInt(st.progress_done||0,10)||0,total=parseInt(st.progress_total||0,10)||0,label='';if(st.probing){label=_('Проверяю всю цепочку прокси');if(st.progress_label)label+=': '+st.progress_label;}else if(st.state==='verifying')label=_('Проверяю, что OpenWrt действительно выходит через локальный шлюз 127.0.0.1:1066');else label=_('Подготавливаю цепочку и запускаю локальный шлюз');var kids=[E('strong',{},label)];if(st.probing&&total>0){kids.push(E('div',{'style':'margin-top:.55em;'},[E('progress',{'max':String(total),'value':String(Math.min(done,total)),'style':'width:100%;max-width:520px;'}),E('span',{'style':'margin-left:.6em;color:#888;'},String(done)+' / '+String(total))]));}return E('div',{'class':'cbi-section','style':'max-width:900px;margin-top:.7em;'},kids);},
	renderBody:function(){
		...
		var statusCard=card(_('OpenWrt Bearhole'),[
			E('p',{'class':'pb-muted'},_('Аварийный системный прокси для самого OpenWrt. При запуске Bearhole автоматически проверяет всю цепочку прокси, оставляет рабочие маршруты, запускает локальный шлюз 127.0.0.1:1066 и только затем использует его для системных загрузок. LAN и маршрутизация Podkop/Forkop не изменяются.')),
			engineWarn,row(_('Состояние'),stateNode(st)),row(_('Шлюз'),E('code',{},st.gateway||'http://127.0.0.1:1066')),row(_('Рабочий маршрут'),E('span',{},st.route_label||st.route_id||'—')),row(_('Последняя проверка маршрутов'),E('span',{},summary)),row(_('Последнее изменение'),E('span',{},age(st.updated_at))),
