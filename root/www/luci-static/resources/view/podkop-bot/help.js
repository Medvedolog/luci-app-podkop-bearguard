		...
		return E('div', {}, [
			...
			card([
				...
				li([
					...
					item(_('Транспорт → WARP'), _('всё про WARP Rescue в одном месте: установка WARPSCOUT, запуск одной кнопкой (учётная запись, поиск узлов и проверка Telegram API выполняются сами), магазин VALID-узлов, ON-AIR, перезарядка, параметры и журналы.')),
