...
return base.constructor.extend({
	...
	/* The base Overview rebuilds the resource card every 5 s. The old code
	 * recreated HWELP/WARP cells as "проверяю…" before each local RPC returned,
	 * causing a visible 5-second flash. Keep the last local state and render it
	 * immediately; RPC completion only changes the value when state really did. */
