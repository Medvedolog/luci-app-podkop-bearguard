...
return base.constructor.extend({
	/* Full batch diagnostics already spend tens of seconds probing every service.
	 * Do not throw that information away and reduce the result to Telegram+speed.
	 * Build one real service matrix from every service returned by active_probe. */
	renderBatch:function(results){
		...
		return E('div',{'class':'cbi-section pb-card','style':'max-width:100%;'},[
			...
			E('p',{'class':'pb-hint-90','style':'margin-top:0;'},_('Показаны все сервисы, которые реально проверила полная диагностика. В ячейке — HTTP-код/задержка; полный статус доступен по наведению.')),
	...
	/* Preserve the disclosure state across the 1.5 s Telegram polling redraws.
	 * During a run it opens automatically; after completion it stays open instead
	 * of collapsing exactly when the user wants to inspect the final table. */
