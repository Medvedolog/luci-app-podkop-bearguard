'use strict';
'require view.podkop-bot.runtime-r41 as base';
'require dom';
...
return base.constructor.extend({
	/* WARP can be fully usable through Rescue even when the discovery status flag
	 * is stale. If Rescue is ON-AIR and shortlist items exist, expose it as a
	 * probeable route instead of silently omitting it from the full test. */
	...
	/* Visual indeterminate bar: the old text-only timer looked frozen during a
	 * 30-90 s probe even though the detached worker was progressing normally. */
	...
	/* Matrix rows are logical routes, not socket diagnostics. Hide proxy addresses
	 * and exit IPs, and collapse duplicate section/tier rows that resolve to the
	 * same local Mixed Proxy endpoint (e.g. akenai == tier1). */
	renderBatch:function(results){
		...
		return E('div',{'class':'cbi-section pb-card','style':'max-width:100%;'},[
			...
			E('p',{'class':'pb-hint-90','style':'margin-top:0;'},_('Одна строка = один логический маршрут. Адреса прокси и IP выхода намеренно скрыты; подробности доступны в одиночной проверке маршрута.')),
