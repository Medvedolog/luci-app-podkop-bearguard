...
function errText(r){var m={forkop_missing:_('Forkop не установлен'),tailscale_unsupported:_('этот sing-box не собран с поддержкой Tailscale/tsnet'),already_configured:_('Tailscale уже настроен'),not_configured:_('Tailscale не настроен'),bad_control_url:_('некорректный URL контрол-сервера'),bad_hostname:_('некорректное имя узла'),auth_key_required:_('нужен pre-auth key'),bad_auth_key:_('некорректный ключ'),standalone_tailscale_running:_('уже работает standalone Tailscale/tailscaled'),uci_write_failed:_('не удалось сохранить UCI'),restart_failed:_('Forkop не перезапустился; изменение отменено'),disable_failed:_('не удалось безопасно выключить endpoint; удаление отменено'),delete_failed:_('не удалось удалить секцию; изменение отменено')};return m[r]||r||'?';}
function standaloneConfirm(){return confirm(_('На роутере уже работает отдельный сервис Tailscale (tailscaled). Одновременный запуск standalone Tailscale и встроенного tsnet sing-box может создать два узла, пересекающиеся маршруты или неожиданный выбор exit node.\n\nПродолжить всё равно?'));}
...
return view.extend({
	...
	body:function(st){
		...
		if(st.standalone_tailscale_present){
			head.push(E('div',{'class':'alert-message '+(st.standalone_tailscale_running?'warning':'notice'),'style':'max-width:850px;'},st.standalone_tailscale_running?_('Обнаружен и запущен standalone Tailscale/tailscaled. Создание или включение встроенного tsnet потребует отдельного подтверждения.'):_('Обнаружен standalone Tailscale, но сейчас он не запущен.')));
		}
	...
	createCard:function(st){
		...
		function runCreate(confirmStandalone){
			btn.disabled=true;dom.content(status,E('em',{},_('Сохраняю секцию…')));
			return callCreate(url.value,key.value,host.value,adv.checked,!!confirmStandalone).then(function(r){
				if(r&&r.reason==='standalone_tailscale_running'&&r.requires_confirmation){
					btn.disabled=false;
					if(!standaloneConfirm())return;
					return runCreate(true);
				}
				if(!r||!r.ok)throw new Error(errText(r&&r.reason));
				return self.refresh();
			}).catch(function(e){dom.content(status,E('span',{'style':'color:#b00;'},_('Ошибка: ')+(e.message||e)));btn.disabled=false;});
		}
		var btn=E('button',{'class':'cbi-button cbi-button-action','click':ui.createHandlerFn(this,function(){
			if(st&&st.standalone_tailscale_running){if(!standaloneConfirm())return;return runCreate(true);}
			return runCreate(false);
	...
	statusCard:function(st){
		...
		function setPower(enable,confirmStandalone){
			power.disabled=true;
			return callSetEnabled(enable,!!confirmStandalone).then(function(r){
				if(r&&r.reason==='standalone_tailscale_running'&&r.requires_confirmation){
					power.disabled=false;
					if(!standaloneConfirm())return;
					return setPower(enable,true);
				}
				if(!r||!r.ok)throw new Error(errText(r&&r.reason));
				return self.refresh();
			}).catch(function(e){dom.content(status,E('span',{'style':'color:#b00;'},_('Ошибка: ')+(e.message||e)));power.disabled=false;});
		}
		var power=E('button',{'class':'cbi-button '+(st.enabled?'cbi-button-negative':'cbi-button-positive'),'click':ui.createHandlerFn(this,function(){
			var enable=!st.enabled;
			if(enable&&st.standalone_tailscale_running){if(!standaloneConfirm())return;return setPower(true,true);}
			return setPower(enable,false);
		})},st.enabled?_('Отключить Tailscale'):_('Подключить к tailnet'));
		...
		return E('div',{'class':'cbi-section','style':'max-width:850px;'},[E('h3',{},_('Узел Tailscale')),
			...
			row(_('Endpoint'),badge(st.enabled,_('включён'),_('выключен'))),row(_('Состояние tsnet'),badge(st.registered,_('state зарегистрирован'),_('state ещё не создан'))),row(_('Standalone Tailscale'),badge(st.standalone_tailscale_running,_('запущен'),st.standalone_tailscale_present?_('установлен, остановлен'):_('не обнаружен'))),row(_('URL авторизации'),auth),
