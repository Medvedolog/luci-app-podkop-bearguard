...
return view.extend({
	...
	render:function(st){
		var self=this;
		this.root=E('div',{});
		dom.content(this.root,this.body(st));
		return this.root;
	},
	...
	body:function(st){
		...
		if(!st.configured){head.push(this.createCard());return E('div',{},head);}
		head.push(this.statusCard(st));return E('div',{},head);
	},
	createCard:function(){
		...
		return E('div',{'class':'cbi-section','style':'max-width:850px;'},[E('h3',{},_('Новый узел')),E('p',{'class':'description'},_('Создаётся выключенным, как и в Telegram-боте. Это не перезапускает Forkop до нажатия «Подключить».')),
	...
	statusCard:function(st){
		var self=this,status=E('div',{'style':'margin-top:.8em;'}),enabled=E('input',{'type':'checkbox','checked':st.enabled?'checked':null}),adv=E('input',{'type':'checkbox','checked':st.advertise_exit_node?'checked':null}),purge=E('input',{'type':'checkbox'});
		var savePower=E('button',{'class':'cbi-button cbi-button-apply','click':ui.createHandlerFn(this,function(){savePower.disabled=true;return callSetEnabled(enabled.checked).then(function(r){if(!r||!r.ok)throw new Error(errText(r&&r.reason));return self.refresh();}).catch(function(e){dom.content(status,E('span',{'style':'color:#b00;'},_('Ошибка: ')+(e.message||e)));savePower.disabled=false;});})},st.enabled?_('Отключить / применить'):_('Подключить к tailnet'));
		...
		return E('div',{'class':'cbi-section','style':'max-width:850px;'},[E('h3',{},_('Узел Tailscale')),
			...
			E('hr'),row(_('Подключение'),E('label',{},[enabled,' ',_('endpoint включён')])),E('div',{'style':'margin:.5em 0 1em;'},[savePower]),
