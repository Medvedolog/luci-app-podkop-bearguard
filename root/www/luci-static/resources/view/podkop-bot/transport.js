...
return view.extend({
	...
	render:function(data){
		...
		var d=this.state;
		if(!d||d.ok===false||d.available===false)return E('div',{},[E('h2',{},_('Транспорт / Спасение')),E('div',{'class':'cbi-section'},dot('grey',_('Состояние транспорта недоступно')))]);
	...
	botTransportCard:function(d){
		...
		var sel=E('select',{'class':'cbi-input-select','style':'padding:.15em .4em;font-size:90%;height:auto;line-height:1.3;max-width:100%;min-width:0;'},[E('option',{'value':'auto'},_('auto (direct → SOCKS)')),E('option',{'value':'socks'},_('только SOCKS')),E('option',{'value':'direct'},_('только direct'))]);sel.value=d.policy||'auto';
		...
		function updateWarn(){warnNode.textContent=(sel.value==='socks'||sel.value==='direct')?_('Режим «')+sel.value+_('» может отключить бота в условиях блокировок.'):'';}sel.addEventListener('change',updateWarn);updateWarn();
		...
		return E('div',{'class':'cbi-section','style':'max-width:820px;border:1px solid var(--border-color-medium,rgba(127,127,127,.2));border-radius:8px;padding:1em 1.2em;background:var(--background-color-high,var(--background-color,var(--background,rgba(40,40,40,.94))));'},[
			...
			E('div',{'style':'display:flex;align-items:center;padding:.3em 0;gap:.5em;flex-wrap:wrap;'},[E('span',{'style':'color:#888;flex:none;'},_('Политика транспорта')),E('span',{'style':'display:flex;align-items:center;gap:.4em;flex-wrap:wrap;'},[sel,saveBtn,policyStatus])]),warnNode
		]);
	},
	...
	buildTiers:function(d){
		...
		(sd.sections||[]).forEach(function(s){
			...
			t.push({id:id,name:'Podkop: '+s.name+' ['+id+']',configured:true,endpoint:s.endpoint,note:_('автоопределён · read-only'),_section:true});
		});
		...
		if(ws.installed&&ws.account_ready&&wc.enabled){
			var parts=[_('настроен'),String(ws.shortlist_count||0)+' '+_('candidate(s)')];
			if(rs.running){parts.push(_('rescue SOCKS активен'));if(rs.endpoint)parts.push(rs.endpoint);}else parts.push(_('rescue SOCKS остановлен'));
	...
	renderTiers:function(tiers,d){
		...
		return E('div',{},tiers.map(function(t){
			...
			else if(t.id==='tier3')extraBtn=E('button',{'class':'cbi-button','style':'padding:.1em .4em;font-size:85%;','title':_('задать/изменить custom proxy'),'click':ui.createHandlerFn(self,'editCustomProxy',t.endpoint)},'✎');
			...
			return E('div',{'style':'padding:.5em .2em;border-bottom:1px solid rgba(127,127,127,.12);'+(isActive?'background:rgba(51,160,44,.06);':'')},[
				E('div',{'style':'display:flex;align-items:center;flex-wrap:wrap;gap:.3em;'},[dotWrap,probeBtn,extraBtn,crudBtns]),probeResult,
	...
	refreshState:function(mutated){
		...
		return Promise.all([callState(),callRuntimeSections().catch(function(){return self.sectionsData;}),callWarpStatus('').catch(function(){return self.warpStatus;}),callRescueStatus().catch(function(){return self.rescueStatus;})]).then(function(res){
			self.state=res[0];self.sectionsData=res[1];self.warpStatus=res[2];self.rescueStatus=res[3];self.tiers=self.buildTiers(self.state);dom.content(self.chainBox,self.renderTiers(self.tiers,self.state));
	...
	fbForm:function(index,current){
		...
		ui.showModal(index<0?_('Добавить резервный прокси'):_('Редактировать резервный прокси'),[E('div',{'class':'pb-manual-proxy-card','style':'border:none;padding:0;max-width:none;'},[E('div',{'class':'pb-manual-proxy-grid'},[scheme,host,port]),E('div',{'class':'pb-manual-proxy-auth'},[user,pass]),E('div',{'style':'margin-top:10px;'},[mnem])]),E('p',{'style':'color:#888;font-size:85%;margin-top:.6em;'},_('Логин/пароль и мнемоника — необязательны. Пароль хранится в конфиге открытым текстом (доступен root).')),err,E('div',{'class':'right','style':'margin-top:.6em;'},[E('button',{'class':'cbi-button','click':ui.hideModal},_('Отмена')),' ',E('button',{'class':'cbi-button cbi-button-apply','click':ui.createHandlerFn(this,submit)},_('Сохранить'))])]);
	},
	...
	editCustomProxy:function(current){
		...
		ui.showModal(_('Свой прокси (tier3)'),[E('div',{'class':'pb-manual-proxy-card','style':'border:none;padding:0;max-width:none;'},[E('div',{'class':'pb-manual-proxy-grid'},[scheme,host,port]),E('div',{'class':'pb-manual-proxy-auth'},[user,pass]),E('div',{'style':'margin-top:10px;'},[mnem])]),E('p',{'style':'color:#888;font-size:85%;margin-top:.6em;'},_('Логин/пароль и мнемоника — необязательны. Пароль хранится в конфиге открытым текстом (доступен root).')),err,E('div',{'class':'right','style':'margin-top:.6em;'},[E('button',{'class':'cbi-button cbi-button-negative','click':ui.createHandlerFn(this,function(){return save(true);})},_('Убрать tier3')),' ',E('button',{'class':'cbi-button','click':ui.hideModal},_('Отмена')),' ',E('button',{'class':'cbi-button cbi-button-apply','click':ui.createHandlerFn(this,function(){return save(false);})},_('Сохранить'))])]);
	},

	editBindIface:function(){var self=this;return callListIfaces().then(function(d){var ifaces=(d&&d.interfaces)||[],current=(d&&d.current)||'',sel=E('select',{'class':'cbi-input-select','style':'width:100%;'},[E('option',{'value':''},_('auto (без привязки)'))].concat(ifaces.map(function(i){return E('option',{'value':i},i);})));sel.value=current||'';var err=E('div',{'style':'color:#cc2b2b;font-size:90%;margin-top:.4em;'});ui.showModal(_('Интерфейс привязки (Direct WAN)'),[E('p',{},_('К какому интерфейсу привязывать прямой выход. auto — выбирает система.')),sel,err,E('div',{'class':'right','style':'margin-top:.6em;'},[E('button',{'class':'cbi-button','click':ui.hideModal},_('Отмена')),' ',E('button',{'class':'cbi-button cbi-button-apply','click':ui.createHandlerFn(self,function(){return callSetField('bind_interface',sel.value).then(function(r){if(r&&r.ok){ui.hideModal();return self.refreshState(true);}dom.content(err,(r&&r.detail)||_('ошибка'));}).catch(function(){dom.content(err,_('ошибка вызова'));});})},_('Сохранить'))])]);}).catch(function(){ui.addNotification(null,E('p',{},_('Не удалось получить список интерфейсов')),'error');});},
