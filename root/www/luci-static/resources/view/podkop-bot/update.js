...
return view.extend({
	...
	render: function(data) {
			...
			'click': ui.createHandlerFn(this, function() {
				...
				.then(function() {
					...
					return callUpdateUpload().then(function(r) {
						...
						dom.content(fileStatus, dot(r.service_running ? 'green' : 'yellow', _('Установлен bot v') + (r.installed_version || '?') + (r.service_running ? _(' · служба запущена') : _(' · служба не запущена'))));
		...
		return E('div', {}, [
			...
			E('p', { 'class':'pb-hint-90', 'style':'max-width:760px;margin-top:0;' }, _('Три независимых модуля: сам веб-интерфейс (LuCI), Telegram-бот и Podkop. Каждый обновляется своим способом.')),
			this.luciCard(),
			E('div', { 'class':'cbi-section', 'style':'max-width:760px;border:1px solid var(--border-color-medium,rgba(127,127,127,.2));border-radius:8px;padding:1em 1.2em;background:var(--background-color-high,var(--background-color,var(--background,rgba(40,40,40,.94))));margin-top:1em;' }, [
				...
				E('div', { 'style':'margin-top:1em;' }, [ E('strong', {}, _('Обновление с GitHub')), E('p', { 'style':'color:#888;font-size:90%;margin:.3em 0;' }, _('Запускает install.sh --action update: скачивает свежий бот с GitHub (с SOCKS-fallback) и устанавливает. Лог установки — ниже.')), ghBtn, ghStatus, ghLog ]),
				E('div', { 'style':'margin-top:1em;border-top:1px solid rgba(127,127,127,.15);padding-top:.8em;' }, [ E('strong', {}, _('Установка из локального файла')), E('p', { 'style':'color:#888;font-size:90%;margin:.3em 0;' }, _('Загружает полный podkop_bot.sh напрямую во временный файл на роутере, проверяет его и устанавливает с резервной копией текущего бота. Лимит: 2 МиБ.')), fileBtn, fileStatus ])
			]),
	...
	pollPodkopLog: function(nodes, btn) {
		...
		var tick = function() { callPodkopLog(self._pkOffset).then(function(r) {
			...
			if (r && r.done) { dom.content(n.status, /podkop-update-exit 0/.test(self._pkLog) ? dot('green', _('Podkop обновлён. Проверьте версию.')) : dot('red', _('Обновление завершилось с ошибкой — см. лог.'))); if (btn) btn.disabled = false; return; }
	...
	pollLuciLog: function() {
		...
		var tick = function() { callLuciLog(self._luciOffset).then(function(r) {
			...
			if (r && r.done) { if (/\[done\] exit 0/.test(self._luciLog)) dom.content(n.status, dot('green', _('Установка завершена. Обновите страницу (Ctrl/Cmd+Shift+R).'))); else if (/\[!!\]/.test(self._luciLog)) dom.content(n.status, dot('red', _('Установка завершилась с ошибкой — см. лог.'))); else dom.content(n.status, dot('green', _('Готово. Обновите страницу.'))); return; }
			setTimeout(tick, 1500);
		}).catch(function(){ setTimeout(tick, 2000); }); };
		tick();
	},
	...
	fillLuci: function(line, actions, d) {
		...
		if (d.latest === 'unknown') { dom.content(line, dot('yellow', _('Текущая: v') + (d.current||'?') + ' · ' + _('последнюю проверить не удалось (ни напрямую, ни через SOCKS).'))); return; }
	...
	fillWarpscout: function(holder, force) {
		...
		callWarpscoutStatus(force).then(function(d) {
			...
			var inner = [ E('h3', { 'style':'margin-top:0;' }, _('WARPSCOUT')), E('p', { 'style':'color:#888;font-size:90%;margin:.3em 0 .7em;' }, _('Опциональный WARP-инструмент. Удаление здесь останавливает WARP и очищает его локальный account/config/shortlist, не затрагивая Podkop и Telegram-бот.')), E('div', { 'class':'pb-row pb-row--plain' }, [ E('span', { 'class':'pb-row-label' }, _('Установлено')), E('span', { 'class':'pb-row-val' }, installed ? dot('green', current) : dot('grey', current)) ]), E('div', { 'class':'pb-row pb-row--plain' }, [ E('span', { 'class':'pb-row-label' }, _('В репозитории')), E('span', { 'class':'pb-row-val' }, latestNode) ]), E('div', { 'class':'pb-action-row', 'style':'margin-top:.7em;display:flex;gap:.5em;flex-wrap:wrap;align-items:center;' }, [ E('a', { 'class':'cbi-button', 'href': (d && d.releases_url) || 'https://github.com/vernette/warpscout/releases', 'target':'_blank', 'rel':'noopener' }, _('Страница WARPSCOUT')), recheck, runBtn, removeBtn ]), opStatus, opLog ];
			dom.content(holder, E('div', { 'class':'cbi-section', 'style':'max-width:760px;border:1px solid var(--border-color-medium,rgba(127,127,127,.2));border-radius:8px;padding:1em 1.2em;background:var(--background-color-high,var(--background-color,var(--background,rgba(40,40,40,.94))));margin-top:1em;' }, inner));
		}).catch(function() { dom.content(holder, E('div', { 'class':'cbi-section', 'style':'max-width:760px;border:1px solid var(--border-color-medium,rgba(127,127,127,.2));border-radius:8px;padding:1em 1.2em;margin-top:1em;' }, [ E('h3', { 'style':'margin-top:0;' }, _('WARPSCOUT')), dot('red', _('backend WARPSCOUT недоступен')) ])); });
	},
	...
	pollWarpscoutLog: function(holder, statusNode, logNode, btn) {
		...
		var tick = function() { callWarpscoutLog(offset).then(function(r) {
			...
			if (r && r.done) { dom.content(statusNode, r.exit_code === 0 ? dot('green', _('WARPSCOUT установлен.')) : dot('red', _('Установка WARPSCOUT завершилась с ошибкой — см. лог.'))); if (btn) btn.disabled = false; setTimeout(function(){ self.fillWarpscout(holder, 'true'); }, 500); return; }
			setTimeout(tick, 1200);
		}).catch(function() { failures++; if (failures >= 10) { dom.content(statusNode, dot('red', _('Не удалось получить временный лог WARPSCOUT'))); if (btn) btn.disabled = false; return; } setTimeout(tick, 1800); }); };
		tick();
	},
	...
	verNode: function(d) {
		...
		if (d.latest === 'unknown') return E('span', {}, [ dot('yellow', _('Текущая: v') + (d.current||'?') + ' · ' + _('последнюю проверить не удалось (ни напрямую, ни через SOCKS).')) ]);
