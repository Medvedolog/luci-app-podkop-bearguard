...
return view.extend({
	...
	render: function(data) {
		...
		return E('div', {}, [
			...
			E('p', { 'class':'pb-hint-90', 'style':'max-width:760px;margin-top:0;' }, _('Четыре независимых модуля, каждый обновляется своим способом: веб-интерфейс LuCI, Telegram-бот, Podkop и WARPSCOUT. Ниже — локальные копии на случай, если GitHub недоступен.')),
			this.luciCard(),
			E('div', { 'class':'cbi-section', 'style':'max-width:760px;border:1px solid var(--border-color-medium,rgba(127,127,127,.2));border-radius:8px;padding:1em 1.2em;background:var(--background-color-high,var(--background-color,var(--background,rgba(40,40,40,.94))));margin-top:1em;' }, [
				E('h3', { 'style':'margin-top:0;' }, _('Telegram-бот')),
				this.currentBlock(),
	...
	fillPodkop: function(holder, force) {
		...
		callPodkopUpdate(force).then(function(d) {
			...
			if (!d || !d.ok || d.available === false) {
				...
				inner = [ E('h3', { 'style':'margin-top:0;' }, d.name || 'Podkop'), E('div', { 'class':'pb-row pb-row--plain' }, [ E('span', { 'class':'pb-row-label' }, _('Вариант')), E('span', { 'class':'pb-row-val' }, d.variant || '—') ]), E('div', { 'class':'pb-row pb-row--plain' }, [ E('span', { 'class':'pb-row-label' }, _('Установлено')), E('span', { 'class':'pb-row-val' }, d.current || '—') ]), E('div', { 'class':'pb-row pb-row--plain' }, [ E('span', { 'class':'pb-row-label' }, _('В репозитории')), E('span', { 'class':'pb-row-val' }, [ upd ? dot('yellow', (d.latest||'—') + _(' — доступно')) : dot('green', (d.latest||'—') + _(' — актуально')) ]) ]), E('div', { 'class':'pb-row pb-row--plain' }, [ E('span', { 'class':'pb-row-label' }, _('Проверено через')), E('span', { 'class':'pb-row-val' }, d.via === 'socks' ? _('прокси SOCKS') : (d.via === 'direct' ? _('напрямую') : '—')) ]), E('p', { 'style':'color:#888;font-size:88%;margin:.5em 0 0;' }, _('Обновление Podkop выполняется его штатным установщиком с GitHub от имени root. Перед запуском проверяются сеть и свободное место.')), (function(){
