...
return view.extend({
	...
	renderBody:function(data){
		...
		var stateNode=rs.running?dot('green',_('активен')):(rs.busy?dot('yellow',String(rs.state||_('работает'))):dot((rs.state==='exhausted'||rs.state==='reload_failed')?'red':'grey',String(rs.state||_('ожидает'))));
		...
		return E('div',{},[
			...
			E('div',{'class':'cbi-section pb-card','style':'max-width:820px;'},[
				...
				E('p',{'class':'pb-muted'},_('«Перезарядить магазин» только подготавливает запас: запускает Discovery, проверяет найденные WARP через Telegram Bot API и собирает новый магазин. WARP при этом сам не запускается. «Запустить лучший» включает первый рабочий вариант, а «Следующий WARP» переходит к следующему.')),
				row(_('WARP Rescue'),enabled?dot('green',_('включён')):dot('grey',_('выключен · магазин всё равно можно подготовить'))),row(_('Состояние'),stateNode),row(_('Магазин'),magNode),row(_('Позиция'),E('span',{},String(rs.index||0)+' / '+String(rs.total||0))),row(_('Активный WARP endpoint'),E('span',{},rs.endpoint||'—')),row(_('Rescue SOCKS'),E('span',{},rs.proxy||('socks5h://127.0.0.1:'+(cfg.socks_port||18191)))),row(_('Автоперезарядка'),auto),
				!enabled?E('p',{'style':'color:#e8a33d;'},_('Чтобы запускать WARP из магазина, включите WARP Rescue в соседней вкладке. Перезарядить магазин можно и сейчас.')):E('span',{}),
				...
				E('div',{'style':'display:flex;gap:.5em;flex-wrap:wrap;'},[saveAuto,act(callRescueTrigger,_('Запустить лучший'),true),act(callRescueNext,_('Следующий WARP'),true),act(callRescueReload,_('Перезарядить магазин'),false),act(callRescueStop,_('Стоп'),false)]),actionStatus,
