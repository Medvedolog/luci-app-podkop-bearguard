...
function rescueError(reason){var m={warpscout_disabled:_('WARP Rescue выключен в настройках'),not_ready:_('WARPSCOUT или account ещё не готовы'),controller_busy:_('револьвер уже выполняет другую команду'),qualification_running:_('сейчас выполняется TG API Routes'),discovery_running:_('сейчас выполняется WARP Discovery'),runtime_handoff_failed:_('не удалось освободить тестовый WARP SOCKS')};return m[reason]||reason||'?';}
...
return view.extend({
	...
	renderBody:function(data){
		...
		var cfg=st.config||{},auto=E('input',{type:'checkbox',checked:rs.auto?'checked':null}),actionStatus=E('div',{'style':'margin-top:.6em;'});
		...
		return E('div',{},[
			...
			E('p',{'class':'pb-muted','style':'max-width:820px;'},_('Револьвер — это быстрый резерв на случай, если обычные маршруты до Telegram перестали работать. Он использует только WARP-адреса, которые уже прошли проверку TG API Routes и признаны VALID.')),
			E('div',{'class':'cbi-section pb-card','style':'max-width:820px;'},[
				E('h3',{'style':'margin-top:0;'},_('WARP Rescue — резервный револьвер')),
				E('p',{'class':'pb-muted'},_('Магазин — список уже проверенных WARP-адресов. «Запустить лучший» поднимает первый рабочий вариант. «Следующий WARP» отбрасывает текущий и пробует следующий. «Перезарядить магазин» заново запускает Discovery, затем проверку Telegram и собирает новый магазин.')),
				row(_('Состояние'),stateNode),row(_('Магазин'),magNode),row(_('Позиция'),E('span',{},String(rs.index||0)+' / '+String(rs.total||0))),row(_('Активный WARP endpoint'),E('span',{},rs.endpoint||'—')),row(_('Rescue SOCKS'),E('span',{},rs.proxy||('socks5h://127.0.0.1:'+(cfg.socks_port||18191)))),row(_('Автоперезарядка'),auto),
				E('p',{'class':'pb-hint-90'},_('Автоперезарядка срабатывает только когда сохранённые VALID WARP закончились: Rescue один раз выполняет Discovery → проверку Telegram → собирает новый магазин. Автоматическое переключение POLL/FAST бота на WARP пока не включено.')),
				E('div',{'style':'display:flex;gap:.5em;flex-wrap:wrap;'},[saveAuto,act(callRescueTrigger,_('Запустить лучший')),act(callRescueNext,_('Следующий WARP')),act(callRescueReload,_('Перезарядить магазин')),act(callRescueStop,_('Стоп'))]),actionStatus,
			...
			E('div',{'style':'margin-top:.7em;display:flex;gap:.5em;flex-wrap:wrap;'},[E('a',{'class':'cbi-button','href':L.url('admin/services/podkop-bot/transport/warpscout')},_('Настройки WARP Rescue')),E('a',{'class':'cbi-button','href':L.url('admin/services/podkop-bot/runtime/tg-api-routes')},_('Открыть TG API Routes'))])
		]);
	},
