	...
	renderBody:function(data){
		...
		return E('div',{},[
			...
			E('div',{'class':'cbi-section pb-card','style':'max-width:820px;'},[
				...
				E('p',{'class':'pb-muted'},_('«Перезарядить магазин» выполняет Discovery, проверяет найденные WARP через Telegram Bot API, собирает магазин и автоматически включает WARP Rescue. Сам туннель при этом не запускается. «Запустить лучший» включает первый рабочий вариант, а «Следующий WARP» переходит к следующему.')),
				...
				E('p',{'class':'pb-hint-90'},_('После успешной перезарядки WARP Rescue включается автоматически. Автоперезарядка срабатывает, когда сохранённые VALID WARP закончились: Rescue один раз выполняет Discovery → проверку Telegram → собирает новый магазин. Автоматическое переключение POLL/FAST бота на WARP пока не включено.')),
