...
function rescueError(reason){var m={warpscout_disabled:_('WARP Rescue выключен'),not_ready:_('WARPSCOUT или account ещё не готовы'),controller_busy:_('револьвер уже выполняет другую команду'),qualification_running:_('сейчас выполняется проверка Telegram API'),discovery_running:_('сейчас выполняется WARP Discovery'),runtime_handoff_failed:_('не удалось освободить служебный WARP SOCKS'),bad_endpoint:_('некорректный WARP endpoint'),not_in_magazine:_('этого endpoint нет в текущем магазине')};return m[reason]||reason||'?';}
...
return view.extend({
	...
	magazineCard:function(mag,rs,actionStatus){
		...
		if(!items.length)return E('div',{'class':'cbi-section pb-card','style':'max-width:820px;'},[E('h3',{'style':'margin-top:0;'},_('Магазин')),E('p',{'class':'pb-hint-90'},_('Магазин пуст. Выполните «Перезарядить»: Discovery → Telegram API → отбор VALID WARP.'))]);
		var body=items.map(function(x){
			...
			var fire=E('button',{
				...
				'click':ui.createHandlerFn(self,function(){
					dom.content(actionStatus,dot('yellow','FIRE · '+x.endpoint+' · запуск SOCKS → Telegram getMe…'));
					fire.disabled=true;
					return callRescueFire(x.endpoint).then(function(r){
						dom.content(actionStatus,r&&r.ok?dot('yellow',r.already_active?_('Уже ON-AIR'):_('Патрон принят · проверяю WARP и Telegram…')):dot('red',_('Ошибка: ')+rescueError(r&&r.reason)));
			...
			if(x.endpoint_ping&&x.endpoint_ping!=='?'&&x.endpoint_ping!=='—')details.push(_('EP ')+x.endpoint_ping);
			if(x.tunnel_ping&&x.tunnel_ping!=='?'&&x.tunnel_ping!=='—')details.push(_('TUN ')+x.tunnel_ping);
			if(x.loss&&x.loss!=='?')details.push(_('loss ')+x.loss);
		...
		return E('div',{'class':'cbi-section pb-card','style':'max-width:820px;'},[E('h3',{'style':'margin-top:0;'},_('Магазин')),E('p',{'class':'pb-hint-90'},_('Только WARP endpoints, прошедшие проверку Telegram API как VALID. FIRE выбирает конкретный патрон и перед ON-AIR ещё раз проверяет Telegram Bot API.')),E('div',{},body)]);
	},
	...
	renderBody:function(data){
		...
		if(rs.busy&&rs.state==='reloading'){
			var phase={manual:_('этап 1/4 · подготовка'),discovery:_('этап 1/4 · Discovery'),qualification:_('этап 2/4 · Telegram API'),building:_('этап 3/4 · сбор магазина')};
		...
		return E('div',{},[
			...
			E('p',{'class':'pb-muted','style':'max-width:820px;'},_('Постоянный резервный WARP SOCKS для Telegram. Использует только WARP-адреса, прошедшие проверку Telegram API как VALID, и работает независимо от открытой вкладки LuCI.')),
			E('div',{'class':'cbi-section pb-card','style':'max-width:820px;'},[
				E('h3',{'style':'margin-top:0;'},_('WARP Rescue')),
				E('p',{'class':'pb-muted'},_('«Перезарядить» выполняет Discovery → Telegram API → сбор магазина → запуск лучшего WARP. «Следующий WARP» переключает Rescue на следующий VALID endpoint.')),
				row(_('WARP Rescue'),enabled?dot('green',_('включён')):dot('grey',_('выключен'))),row(_('Состояние'),stateNode),row(_('Магазин'),magNode),row(_('Позиция'),E('span',{},position)),row(_('Активный WARP endpoint'),E('span',{},rs.endpoint||'—')),row(_('Rescue SOCKS'),rs.running?dot('green',(rs.proxy||('socks5h://127.0.0.1:'+(cfg.socks_port||18191)))):E('span',{},rs.proxy||('socks5h://127.0.0.1:'+(cfg.socks_port||18191)))),row(_('Автоперезарядка при исчерпании магазина'),auto),
				E('p',{'class':'pb-hint-90'},_('Если все сохранённые VALID WARP исчерпаны, автоперезарядка заново выполняет Discovery → Telegram API → сбор магазина и пытается поднять новый рабочий Rescue SOCKS.')),
