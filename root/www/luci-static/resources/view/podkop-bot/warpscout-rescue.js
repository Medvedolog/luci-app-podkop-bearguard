...
function rescueError(reason){var m={warpscout_disabled:_('WARP Rescue выключен'),not_ready:_('WARPSCOUT или account ещё не готовы'),controller_busy:_('револьвер уже выполняет другую команду'),qualification_running:_('сейчас выполняется TG API Routes'),discovery_running:_('сейчас выполняется WARP Discovery'),runtime_handoff_failed:_('не удалось освободить служебный WARP SOCKS')};return m[reason]||reason||'?';}
...
return view.extend({
	...
	renderBody:function(data){
		...
		var stateNode=rs.running?dot('green',_('работает')):(rs.busy?dot('yellow',busyLabel):dot((rs.state==='exhausted'||rs.state==='reload_failed')?'red':'grey',enabled?_('не запущен'):String(rs.state||_('остановлен'))));
		...
		return E('div',{},[
			...
			E('div',{'style':'margin-top:.7em;display:flex;gap:.5em;flex-wrap:wrap;'},[E('a',{'class':'cbi-button','href':L.url('admin/services/podkop-bot/transport/warpscout')},_('Расширенные настройки WARP Rescue')),E('a',{'class':'cbi-button','href':L.url('admin/services/podkop-bot/runtime/tg-api-routes')},_('Открыть TG API Routes')),E('a',{'class':'cbi-button','href':L.url('admin/services/podkop-bot/update')+'#warpscout-update'},_('Установка / удаление WARPSCOUT'))])
		]);
	},
