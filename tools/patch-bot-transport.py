#!/usr/bin/env python3
"""Patch the vendored bot at staging time with the 0.19.18 transport fix.

The vendored bot is intentionally kept byte-identical to the standalone source
until the standalone dev branch is synchronized.  The package payload, however,
must not regress to Direct while the independent Telegram follower still proves
proxy reachability.  This patch also promotes an active WARP Rescue SOCKS to a
real bot transport before Direct.
"""
from pathlib import Path
import sys

if len(sys.argv) != 2:
    raise SystemExit("usage: patch-bot-transport.py <podkop_bot>")
...
# Package and bot report the same tested transport generation.
...
# WARP Rescue is a local SOCKS provider managed by the existing revolver RPC.
# It remains optional: no WARPSCOUT installation/config means this helper is a
# no-op and the legacy chain behaves exactly as before.
warp_helpers = r'''
...
# Print active Rescue proxy URL.  With $1=1, request the existing Rescue
# controller to start it when the user has Rescue enabled but the SOCKS process
# is currently absent.  Triggering is throttled so repeated failed POLLs cannot
# hammer ubus or continuously restart WARPSCOUT.
_warp_rescue_proxy() {
...
# Real chain: Podkop/section/fallback -> custom -> WARP Rescue -> Direct.
replace(
...
old_hysteresis = '''    # fresh positive proxy sample, one failed long-poll cascade is treated as transient.
...
'''
new_hysteresis = '''    # least one proxy route can reach Telegram with a fresh getMe, a failed 50s
...
'''
replace(old_hysteresis, new_hysteresis)

# Sticky WARP route is retried as itself instead of being mistaken for custom or
# Direct.  If the revolver process died, _warp_rescue_proxy(1) may re-arm it.
replace(
'''            tier4)
...
# Direct/Emergency recovery probes must include Rescue before remaining degraded.
...
# Treat WARP Rescue as a healthy proxy route everywhere the watchdog currently
# recognises tier1/tier2/tier3 as healthy.
s = s.replace('tier1|tier2_*|tier3)', 'tier1|tier2_*|tier3|warp_rescue)')

# Follower measures the active Rescue endpoint too.  This gives POLL the same
# independent short Telegram proof for WARP that it already has for normal proxy
# tiers.
replace(
'''    if [ -n "$_t_custom" ]; then
        _slots="$_slots tier3"
...
# sendDocument follows the same route order as FAST/POLL.
replace(
'''        if [ -n "$_t_custom" ]; then
...
# Latency/status helpers understand the new sticky route key.
replace(
'''        tier4)
            p_args="$if_flag"
            ;;
...
# Keep comments truthful; these are useful when support bundles include source.
s = s.replace(
